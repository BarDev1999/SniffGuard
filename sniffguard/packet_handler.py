...
def process_packet(self, packet):
    ...
    for detector in self.detectors:
        ...
        # 3. אם חזרה התראה, עדכן UI וצא מהלולאה
        if alert_details:
            display_timestamp = time.strftime("%H:%M:%S")
            attacker = alert_details['attacker']
            evidence_suffix = alert_details['evidence_suffix']

            # בדוק אם זה SYN scan או סוג אחר
            scan_name = alert_details.get("scan_name", "PORT SCAN") # "PORT SCAN" כברירת מחדל
